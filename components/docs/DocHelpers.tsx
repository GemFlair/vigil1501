...
export const SectionHeader: React.FC<{ id: string, category: string, title: string, subtitle: string, colorClass?: string, bgGlow?: string }> = ({ id, category, title, subtitle, colorClass = "text-blue-500", bgGlow = "bg-blue-600/5" }) => (
  <header className="relative z-10 p-[1px] bg-gradient-to-br from-zinc-700 to-transparent rounded-[1.5rem] md:rounded-[28px] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)] overflow-hidden mb-8 md:mb-20">
    <div className={`absolute top-0 right-0 w-[200px] md:w-[500px] h-[200px] md:h-[500px] ${bgGlow} blur-[80px] md:blur-[120px] rounded-full`} />
    <div className="bg-[#080808]/90 backdrop-blur-3xl rounded-[1.4rem] md:rounded-[27px] px-3 py-8 md:p-20 space-y-4 md:space-y-10 relative border border-white/10">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 md:gap-12">
        <div className="space-y-4 md:space-y-8">
          <div className="flex flex-wrap items-center gap-3 md:gap-4">
             ...
             <div className="px-3 md:px-5 py-1 bg-zinc-900 border border-zinc-700 rounded-lg text-[8px] md:text-[10px] font-mono text-zinc-500 uppercase tracking-widest">{id}</div>
          </div>
...
export const TechNote: React.FC<{ children?: React.ReactNode, title: string }> = ({ children, title }) => (
  <div className="px-4 py-6 md:p-8 bg-zinc-950/50 border border-zinc-800 rounded-[1.25rem] md:rounded-[28px] space-y-3">
...
export const ClauseFrame: React.FC<{ children?: React.ReactNode, id: string }> = ({ children, id }) => (
  <div className="relative pl-4 md:pl-12 border-l border-zinc-700 py-4 group">
    <div className="absolute left-0 top-6 w-3 md:w-8 h-[1px] bg-zinc-700 group-hover:bg-blue-600/50 transition-colors" />
    <div className="absolute left-[-0.65rem] md:left-[-1.5rem] top-5 px-1 md:px-1.5 py-0.5 bg-zinc-950 border border-zinc-800 rounded text-[6px] md:text-[8px] font-mono text-zinc-600 font-black group-hover:text-blue-500 group-hover:border-blue-900/50 transition-all">
      {id}
    </div>
