...
import { 
  ...
  MessageSquare, Flame, X, Lock, ChevronRight, Trash2, RefreshCw, Zap, Monitor
} from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ 
  activeAnchor, scrollToSection, isMenuOpen, setIsMenuOpen, releasePhase, onCodeSubmit, ambientStatus, isAdmin = false, bri, xp, rank, onOpenMap
}) => {
  ...
  const navItems: NavItem[] = [
    ...
    { type: 'header', label: 'Phase: Simulation', level: 3 },
    { id: 'hub-execution', label: 'Hub 01: Sandbox', icon: <TerminalIcon size={14} />, level: 4, type: 'item' },
    { id: 'intent-validator-demo', label: 'P1: Intent Validator', icon: <Target size={12} />, level: 4, type: 'subitem' },
    { id: 'mimicry-lab-demo', label: 'P2: Mimicry Lab', icon: <Fingerprint size={12} />, level: 4, type: 'subitem' },
    
    { id: 'hub-synthesis', label: 'Hub 02: Synthesis', icon: <Globe size={14} />, level: 4, type: 'item' },
    { id: 'intel-forge-demo', label: 'P3: Intelligence Forge', icon: <Cpu size={12} />, level: 4, type: 'subitem' },
    { id: 'reputation-search', label: 'P4: Reputation Search', icon: <Search size={12} />, level: 4, type: 'subitem' },
    { id: 'sentinel-deck-demo', label: 'P5: Sentinel Deck', icon: <Layout size={12} />, level: 4, type: 'subitem' },
    
    { id: 'hub-biological', label: 'Hub 03: Biological', icon: <Brain size={14} />, level: 4, type: 'item' },
    { id: 'neural-firewall-demo', label: 'P6: Saccadic Trainer', icon: <Activity size={12} />, level: 4, type: 'subitem' },
    { id: 'neural-audit-demo', label: 'P7: Saccadic Depth', icon: <Eye size={12} />, level: 4, type: 'subitem' },

    { id: 'hub-apex', label: 'Hub 04: Apex', icon: <Zap size={14} className="text-amber-500" />, level: 4, type: 'item' },
    { id: 'entropy-collider-demo', label: 'P8: Entropy Collider', icon: <Activity size={12} className="text-amber-500" />, level: 4, type: 'subitem' },
    ...
    { id: 'neural-audit', label: 'Final Proficiency', icon: <Activity size={14} />, level: 8, type: 'item' },
  ...
  useEffect(() => {
    if (!activeAnchor || !scrollableNavRef.current) return;
  ...
  return (
    <aside className={`fixed inset-x-0 bottom-0 top-10 z-[100] bg-[#050505] md:relative md:top-0 md:h-full w-full md:w-72 border-r border-zinc-800 transition-all duration-500 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
      <div className="flex flex-col h-full">
        ...
        <div ref={scrollableNavRef} className="flex-1 min-0 relative overflow-y-auto custom-scrollbar px-6 pt-1 pb-4">
          <nav className="space-y-1 pb-10">
            {navItems.map((item, idx) => {
              ...
              return (
                <button key={item.id || idx} data-active={isActive} disabled={isLocked} onClick={() => scrollToSection(item.id || '')} className={`w-full text-left px-4 py-3 rounded-2xl transition-all duration-500 flex items-center group relative border ${isActive ? 'bg-[#0a0a0a] border-zinc-700 scale-[1.02]' : isLocked ? 'opacity-40 cursor-not-allowed border-transparent' : 'text-zinc-500 border-transparent hover:text-zinc-200 hover:bg-zinc-950/40'} ${isSubItem ? 'ml-4 py-2' : ''}`}>
                  <div className={`rounded-xl border flex items-center justify-center mr-4 shrink-0 ${isActive ? 'text-blue-500 bg-blue-500/10 border-blue-500/30' : 'text-zinc-700 border-zinc-800'}`}>
                    {isLocked ? <Lock size={isSubItem ? 10 : 12} /> : item.icon}
                  </div>
                  <span className={`font-black tracking-[0.2em] uppercase transition-all ${isActive ? 'text-white' : ''} ${isSubItem ? 'text-[9px]' : 'text-[11px]'}`}>
                  ...
                  {isSubItem && !isLocked && (<div className="absolute left-[-1rem] top-1/2 -translate-y-1/2 w-4 h-[1px] bg-zinc-700" />)}
                </button>
              );
            })}
          </nav>
        ...
        <div className="p-6 border-t border-zinc-800 shrink-0 space-y-4 bg-[#050505] z-30 shadow-2xl">
          <div className="space-y-2">
            ...
            <form onSubmit={handleSubmitCode} className="relative group">
