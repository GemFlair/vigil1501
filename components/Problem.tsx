...
export const Problem: React.FC<{ onOpenDoc?: (doc: RegistryDoc) => void }> = ({ onOpenDoc }) => {
  ...
  return (
    <section id="the-threat" className="flex flex-col items-center justify-center p-6 md:p-12 py-8 relative overflow-hidden bg-[#020202]">
      <div ref={containerRef} className="w-full max-w-6xl space-y-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {matrixCards.map((card, i) => (
            <div 
              ...
              className={`p-10 bg-zinc-950/40 border border-zinc-700 rounded-[16px] transition-all duration-1000 relative overflow-hidden will-change-transform ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
              } hover:border-zinc-500 hover:-translate-y-1.5 group`}
            >
              <div className="w-14 h-14 bg-zinc-900/50 rounded-[12px] flex items-center justify-center mb-8 border border-zinc-800 group-hover:scale-110 transition-transform duration-500">{card.icon}</div>
        ...
        <div className={`p-12 md:p-16 bg-blue-600/5 border border-blue-500/30 rounded-[24px] md:rounded-[32px] text-center relative group overflow-hidden transition-all duration-1000 delay-[800ms] ${isVisible ? 'opacity-100' : 'opacity-0 scale-[0.98]'}`}>
          <p className="text-xl md:text-2xl font-bold text-white leading-relaxed italic relative z-10 max-w-4xl mx-auto tracking-tight">
