...
const FeatureCard: React.FC<FeatureCardProps> = ({ feature, index, delayOffset }) => {
  ...
  return (
    <div 
      ...
      className={`group p-10 bg-[#080808] border border-zinc-700 rounded-[16px] relative h-full transition-all duration-[600ms] will-change-transform overflow-hidden ${
    ...
    >
      ...
      {feature.isPrivacyFirst && (
        <div className={`absolute top-8 right-10 z-20 transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: `${cardDelay + 300}ms` }}>
          <div className="px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/30 rounded-lg text-[9px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-1.5">
      ...
      <div className={`relative z-10 space-y-6 ${isVisible ? 'animate-float' : ''}`} style={{ animationDelay: `${cardDelay}ms` }}>
        <div className={`w-12 h-12 bg-zinc-950 border border-zinc-700 rounded-[12px] flex items-center justify-center transition-all duration-500 group-hover:bg-zinc-900 ${
        ...
        <div className="space-y-3">
          <h3 className="text-xl font-black text-white uppercase italic tracking-tight group-hover:text-white transition-colors">{feature.title}</h3>
          <p className="text-zinc-500 text-[13px] md:text-lg leading-relaxed font-medium transition-colors group-hover:text-zinc-400">{feature.description}</p>
        </div>
      </div>
...
export const Features: React.FC<{ onOpenDoc?: (doc: RegistryDoc) => void }> = ({ onOpenDoc }) => {
  ...
  const coreFeatures = [
    { icon: <Search className="w-5 h-5 text-blue-500" />, title: "ADDRESS SIMILARITY DETECTION", description: "VIGIL compares the destination address against your recent history and warns you when a visually similar address appears — stopping copy-paste scams and impersonation risks.", accentColor: "blue" },
    { icon: <AlertCircle className="w-5 h-5 text-emerald-500" />, title: "DUST TRANSFER ALERTS", description: "Unexpected zero-value or tiny transfers can be a setup for address poisoning. VIGIL flags these patterns so you don’t accidentally copy a malicious sender.", accentColor: "emerald" },
    { icon: <CheckCircle2 className="w-5 h-5 text-purple-400" />, title: "FIRST-TIME CONFIRMATION", description: "Sending funds to a new address deserves extra attention. VIGIL provides a gentle confirmation step to help you pause and verify before proceeding.", accentColor: "purple" }
  ];

  const advancedFeatures = [
    { icon: <ShieldCheck className="w-5 h-5 text-purple-500" />, title: "FAKE TOKEN VERIFICATION", description: "Scammers often use fake USDT or USDC tokens to poison transaction history. VIGIL verifies token mint authenticity to distinguish real assets from impersonations.", accentColor: "purple" },
    { icon: <Database className="w-5 h-5 text-blue-400" />, title: "POISON SENDER MEMORY", description: "Addresses previously involved in poisoning attempts are remembered locally in your browser, allowing VIGIL to warn you if they appear again.", accentColor: "blue" },
  ...
  return (
    <div ref={sectionRef} className="space-y-8 md:space-y-12 py-8 overflow-x-hidden">
      <section id="features" className="px-6 md:px-20 max-w-7xl mx-auto scroll-mt-20">
        <div className="text-center space-y-6 mb-12 md:mb-16">
          ...
          <h2 className={`text-5xl md:text-7xl font-black text-white italic uppercase tracking-tighter leading-none transition-all duration-700 delay-[100ms] ${headerVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>Your Wallet Safety Co-pilot</h2>
          <div className={`space-y-8 max-w-3xl mx-auto transition-all duration-700 delay-[200ms] ${headerVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
            <p className="text-zinc-400 text-lg md:text-xl font-medium leading-relaxed">VIGIL adds a quiet awareness layer that reviews addresses and transaction context before you sign, helping you catch copy-paste scams and impersonation risks.</p>
          </div>
        </div>
        ...
        <div className="mb-10 relative">
           ...
           <div className={`text-[10px] font-black uppercase tracking-[0.4em] mb-8 flex items-center gap-4 transition-all duration-700 delay-[400ms] ${headerVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
             <div className="px-4 py-2.5 glass bg-emerald-500/10 border border-emerald-500/40 rounded-xl flex items-center gap-3 relative z-10">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">{coreFeatures.map((feature, i) => (<FeatureCard key={i} feature={feature} index={i} delayOffset={100} />))}</div>
        </div>
        ...
        <div className="relative">
           ...
           <div className={`text-[10px] font-black uppercase tracking-[0.4em] mb-8 flex items-center gap-4 transition-all duration-700 delay-[600ms] ${headerVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
             <div className="px-4 py-2.5 glass bg-blue-500/10 border border-blue-500/40 rounded-xl flex items-center gap-3 relative z-10">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">{advancedFeatures.map((feature, i) => (<FeatureCard key={i} feature={feature} index={i} delayOffset={100} />))}</div>
        </div>
      </section>
    </div>
  );
