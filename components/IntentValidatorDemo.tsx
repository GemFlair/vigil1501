...
export const IntentValidatorDemo: React.FC<IntentValidatorDemoProps> = ({ onUsageUpdate, onScanningChange }) => {
  ...
  const [result, setResult] = useState<(ThreatAnalysisResponse & { telemetry?: { age: string; lastTx: string; activity15d: string; latency?: number }; threatIndex?: number; axes?: TIMAxes; }) | null>(null);
  ...
  const testScenarios = [
    ...
    { id: 'MARKET_INTEL', label: 'Pump.fun Rug-Risk', addr: 'Rug44DeployerX992811x772199291120038', telemetry: { age: '4 Minutes', lastTx: 'New', activity15d: '142' }, axes: { vsi: 20, edi: 98, pdi: 100, cri: 90, ipi: 80, rii: 0, eii: 0 } },
  ...
  const handleValidate = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        setResult({ ...data, intentState: (matchedScenario?.id as IntentCategory) || data.intentState, telemetry: { ...(matchedScenario?.telemetry || { age: data.onChainAge, lastTx: 'Unknown', activity15d: '0' }), latency: latency }, threatIndex: threatIndex, axes: matchedScenario?.axes || { vsi: data.riskScore, edi: 10, pdi: 20, cri: 10, ipi: 5, rii: 0, eii: 0 } });
