...
const VisualDeceptionShield = () => (
  <div className="p-6 md:p-14 bg-[#080808] border-2 border-zinc-700 rounded-[24px] md:rounded-[32px] relative overflow-hidden group shadow-2xl transition-all duration-700 hover:border-red-600/25">
...
const WalletSimulator = ({ onUnlock, currentLevel, onFail, onOpenMap, onIntegratedChange }: { onUnlock: () => void, currentLevel: number, onFail: () => void, onOpenMap: () => void, onIntegratedChange: (val: boolean) => void }) => {
  ...
  return (
    <div className={`transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${isShattering ? 'scale-125 opacity-0' : 'opacity-100'}`} style={{ perspective: '2000px', transformStyle: 'preserve-3d' }}>
      <div className={`p-6 md:p-14 bg-[#0a0a0a] border transition-all duration-700 rounded-[24px] md:rounded-[32px] relative overflow-hidden group shadow-2xl ${isDecrypted ? 'border-emerald-500/45 shadow-[0_0_80px_rgba(16,185,129,0.05)]' : 'border-zinc-700'} ${isShattering ? 'pointer-events-none' : ''}`} style={{ transformStyle: 'preserve-3d' }}>
        {isShattering && (
          ...
          <div className="space-y-8 md:space-y-10 animate-in zoom-in duration-500 pt-10 md:pt-12 relative z-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                {!isDecrypted && !autopsyActive && (
                  <div className="flex items-center gap-3 px-5 py-3 bg-blue-600/10 border border-blue-500/25 rounded-[12px] animate-pulse w-fit">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10">
                <div className="lg:col-span-7 space-y-6 md:space-y-8">
                   {!autopsyActive && !isDecrypted ? (
                     <div className="space-y-6 md:space-y-8">
                        <div className="p-6 md:p-8 bg-black border border-zinc-700 rounded-[16px] space-y-5 relative group overflow-hidden">
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="grid grid-cols-1 gap-3">
                              <button onClick={() => handleIntercept('POISON')} className="p-4 md:p-6 bg-zinc-900/50 border border-zinc-700 rounded-[16px] flex items-center justify-between group/btn hover:border-red-500/35 transition-all active:scale-[0.98]">
                              ...
                              <button onClick={() => handleIntercept('FRIEND')} className="p-4 md:p-6 bg-zinc-900/50 border border-zinc-700 rounded-[16px] flex items-center justify-between group/btn hover:border-emerald-500/35 transition-all active:scale-[0.98]">
                ...
                <div className="lg:col-span-5 flex flex-col gap-4">
                   <div className="flex-1 p-6 md:p-8 bg-black/40 border border-zinc-700 rounded-[16px] space-y-6 flex flex-col justify-center">
                   ...
                   <button onClick={onOpenMap} className="p-6 md:p-8 bg-blue-600/5 border border-blue-500/25 rounded-[16px] flex items-center justify-between group hover:bg-blue-600/10 transition-all">
                      <div className="flex items-center gap-4">
